...
static int zns_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    if (zone_cap > zone_size) {
        femu_err("zone capacity %luB > zone size %luB\n", zone_cap, zone_size);
        return -1;
    }
    if (zone_size < lbasz) {
        femu_err("zone size %luB too small, must >= %uB\n", zone_size, lbasz);
        return -1;
    }
    if (zone_cap < lbasz) {
        femu_err("zone capacity %luB too small, must >= %uB\n", zone_cap, lbasz);
        return -1;
    }
                             ...
                             bool wrz)
{
    ...
    // femu_err("zone append doesn't work ! status=(%d) in zone.slba=%ld\n",status, zone->d.zslba);
    if (status) {
        femu_err("%s : error here3\n", __func__);
        goto err;
    }

    status = zns_auto_open_zone(ns, zone);
    if (status) {
        femu_err("%s : error here4\n", __func__);
        goto err;
    }
                          ...
                          NvmeRequest *req)
{
    ...
    uint64_t slba = le64_to_cpu(rw->slba);//起始逻辑块地址
    uint32_t nlb = (uint32_t)le16_to_cpu(rw->nlb) + 1;// datasize的逻辑块数量
    uint64_t data_size = zns_l2b(ns, nlb);// 逻辑块数量转化为字节数
    uint64_t data_offset;
    NvmeZone *zone;
    NvmeZonedResult *res = (NvmeZonedResult *)&req->cqe;
    uint16_t status;
    ...
    zone = zns_get_zone_by_slba(ns, slba);// 获取zone
...
static void znsssd_init_params(FemuCtrl * n, struct zns_ssdparams *spp){
    ...
    */
    ...
    spp->csze_pages     = (((int64_t)n->memsz) * 1024 * 1024) / MIN_DISCARD_GRANULARITY / spp->nchnls / spp->ways;// 一个way管多少个页
    spp->nchips         = (((int64_t)n->memsz) * 1024 * 1024) / MIN_DISCARD_GRANULARITY / spp->csze_pages;        // 多少个way，也就是多少个chip，其实就是nchnls * ways
}
